...
public class Simpletron_Processor {
    ...
    public void run() {
        running = true;
        while (running && pc < memory.getSize()) {
            String instruction = memory.getitem(pc);
            if (instruction == null || instruction.equals("0000")) {
                pc++; // NOP, just continue
                continue;
            }
    ...
    private void decode(String opcode, String operand) {
        ...
        switch (opcode) {
            ...
            case "37": // DIV immediate
                if (address == 0) throw new ArithmeticException("Division by zero");
                accumulator /= address;
                break;
